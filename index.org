...
*** Lots Of Ways To Install Packages
    ...
    packages out-of-the-box that many other distros do not (e.g. Zoom).

    So I started installing a bunch of packages that I needed using this tool, like
    Emacs and Zoom. I then opened the terminal app and tried launching Zoom like this:
...
#+END_EXAMPLE
    ...
    Uh, this is weird. So it turned out that everything that I had installed using
    AppCenter was installed using [[https://flatpak.org/][Flatpak]]. It's a new format that's kindof like [[https://snapcraft.io/store][Snaps]]
    or [[https://appimage.org/][AppImage]] ([[https://github.com/AppImage/AppImageKit/wiki/Similar-projects#comparison][here's]] an excellent comparison of the 3). 
    ...
    Also, the location of config files changes. I installed [[https://www.thunderbird.net/en-US/][Thunderbird]] using the
    AppCenter because the version available via =apt= was too old for my profile. For
    as long as I've used Thunderbird the config files have always been under
    =~/.thunderbird=. How they're in =~/.var/app/org.mozilla.Thunderbird/=. Not a
    huge deal but something that tripped me up a little bit.

    So is that bad? Well, not for most of the apps that I would install using the
    AppCenter. But if I ever want to run the app from the command line then I think
    it pays to install the app using =apt= instead.
*** Flatpak Advantages
